...
class MainWindowClass(QMainWindow):
    def __init__(self):
        ...
        # FIX: Do NOT pass 'self' as parent for QMainWindow .ui files
        ...
        # Re-parent the loaded UI to this window
    ...
    def add_assistant_message(self, message: str):
        self.chat_display.append(f"<b>🤖 Assistant:</b><br>{self.format_code_blocks(message)}")
        self.scroll_to_bottom()
    ...
    def remove_typing_indicator(self):
        ...
        if text.endswith("🤖 Assistant is typing..."):
    ...
    def on_stream_chunk(self, chunk: str):
        if self.chat_display.toPlainText().endswith("🤖 Assistant is typing..."):
    ...
    def format_code_blocks(self, text: str) -> str:
        ...
        pattern = r'```(\w*)\n(.*?)```'
        replacement = r'<pre style="background-color: #1e1e1e; border-left: 3px solid #0078d4; padding: 10px; border-radius: 5px; overflow-x: auto;"><code style="font-family: Consolas, monospace;">\2</code></pre>'
        text = re.sub(pattern, replacement, text, flags=re.DOTALL)
        
        text = re.sub(r'`(.*?)`', r'<code style="background-color: #3c3c3c; padding: 2px 4px; border-radius: 3px;">\1</code>', text)
